...
The **cronjob — native systemd Linux** subpackage includes **Schedule Codex Hi**, **Schedule Claude Hi**, and **Schedule Antigravity Hi**. Select **Run** for the client you want and enter a daily local time in `HH:MM` format, or accept the `07:00` default. Each installer runs in the foreground, enables and starts an existing `cron.service` or `crond.service` using `sudo` when needed, and installs one user crontab entry for every day of the week. The three installers, schedules, configuration directories, status commands, and cron blocks are independent: installing, reconfiguring, or removing one does not change the others. Setup requires the matching CLI to be installed and logged in, `crontab`, systemd, `flock`, and standard GNU/Linux utilities; it does not install OS packages, invoke the client, or open the status viewer. These jobs require native systemd Linux; WSL is unsupported.

The scheduled requests are fixed by client: Codex runs `codex exec --ephemeral --skip-git-repo-check --sandbox read-only --color never Hi`, Claude runs `claude --print --no-session-persistence --permission-mode plan Hi`, and Antigravity runs `agy --print --mode plan Hi`. The machine must be awake with cron running at the scheduled local time; normal cron timezone and daylight-saving behavior applies. Runs are not retried, and overlapping runs for the same client are skipped. A zero client exit status is recorded as success; non-zero exits are recorded as errors, with client output written to that client's latest-run log.

Run `codex-hi`, `claude-hi`, or `agy-hi` to view that client's schedule, recent timestamped events, and continuous status updates until Ctrl+C. The viewer reports schedule and scheduler health, including whether the cron service is active/enabled and whether its cron entry is present; it checks scheduler health every 30 seconds. Run events include submission, client output, success or failure, and waiting for the next run. Each command is installed at `~/.local/bin/<client>-hi`; add `~/.local/bin` to `PATH` if needed. Runtime files are stored under `${XDG_DATA_HOME:-$HOME/.local/share}/<client>-hi`, and event/latest-run logs under `${XDG_STATE_HOME:-$HOME/.local/state}/my-toolbox/<client>-hi`.

Re-running an installer changes only that client's daily schedule and preserves its history. `tb uninstall` leaves these jobs installed. To remove one job, run its command with `--uninstall` (for example, `claude-hi --uninstall`); this removes only that client's schedule, wrapper, configuration, and logs, while leaving the shared cron service, unrelated crontab entries, and the other two jobs intact. A downloaded installer can be run as `sh codex_hi.sh 07:00`, `sh claude_hi.sh 07:00`, or `sh agy_hi.sh 07:00`; omit the time to be prompted.
