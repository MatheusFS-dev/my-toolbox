...
# Keep the source ASCII-only for Windows PowerShell 5.1, which decodes
# BOM-less scripts with the active legacy code page.
$Banner = @'
###>   ###>##>   ##>    ########> ######>  ######> ##>     ######>  ######> ##>  ##>
####> ####|<##> ##+]    <==##+==]##+===##>##+===##>##|     ##+==##>##+===##><##>##+]
##+####+##| <####+]        ##|   ##|   ##|##|   ##|##|     ######+]##|   ##| <###+]
...
<=]     <=]   <=]          <=]    <=====]  <=====] <======]<=====]  <=====] <=]  <=]
'@
$Banner = $Banner.Replace('#', [char]0x2588).Replace('>', [char]0x2557).Replace('<', [char]0x255A).
