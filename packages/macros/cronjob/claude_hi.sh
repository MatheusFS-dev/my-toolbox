#!/bin/sh
# my-toolbox claude-hi managed runtime v1
set -eu
umask 077
...
data_root="${XDG_DATA_HOME:-$HOME/.local/share}/claude-hi"
state_base="${XDG_STATE_HOME:-$HOME/.local/state}/my-toolbox"
state_root="$state_base/claude-hi"
lifecycle_lock="$state_base/cronjob.lifecycle.lock"
wrapper="$HOME/.local/bin/claude-hi"
start_marker='# >>> my-toolbox claude-hi >>>'
end_marker='# <<< my-toolbox claude-hi <<<'
mode=${1:-}
...
if [ "$mode" = --runtime ]; then
    ...
    export PATH HOME
    mode=${1:---watch}
    [ "$#" -le 1 ] || fail 'Usage: claude-hi [--uninstall]'
    case "$mode" in --watch|--run|--uninstall) ;; *) fail 'Usage: claude-hi [--uninstall]';; esac
fi
...
read_crontab() {
    ...
    ' "$transaction/original" > "$transaction/clean" || fail 'Malformed claude-hi crontab markers.'
}
...
case "$mode" in
    --run)
        ...
        finish() {
            result=$?
            trap - 0
            if [ "$result" -eq 0 ]; then event 'SUCCESS: Claude exited successfully.'
            else event "ERROR: Claude exit $result; log: $state_root/latest-run.log"; fi
    ...
    --uninstall)
        ...
        exec 9> "$state_root/run.lock"
        flock -n 9 || fail 'A Claude run is active; retry uninstall when it finishes.'
        ...
        grep -q '^# my-toolbox claude-hi wrapper v1$' "$wrapper" && rm -f "$wrapper"
        ...
        say 'Uninstalled claude-hi and removed its logs; shared cron service left running.'
        exit 0
        ;;
    --*) fail 'Usage: claude-hi [--uninstall]';;
esac

[ "$#" -le 1 ] || fail 'Usage: sh claude_hi.sh [HH:MM]'
schedule=${1:-}
if [ -z "$schedule" ]; then
...
claude_path=$(command -v claude) || fail 'Install Claude and log in before configuring this job.'
case "$claude_path" in /*) ;; *) fail 'Claude must resolve to an absolute executable path.';; esac
cron_unit=
...
if [ -e "$wrapper" ]; then
    if [ ! -f "$wrapper" ] || ! grep -q '^# my-toolbox claude-hi wrapper v1$' "$wrapper"; then fail "Unmanaged command: $wrapper"; fi
fi
if [ -e "$data_root" ]; then
    if [ ! -f "$data_root/runtime.sh" ] || ! grep -q '^# my-toolbox claude-hi managed runtime v1$' "$data_root/runtime.sh"; then
...
flock -n 8 || fail 'Another installation is running.'
exec 9> "$state_root/run.lock"
flock -n 9 || fail 'A Claude run is active; retry configuration later.'
exec 7> "$lifecycle_lock"
flock -n 7 || fail 'Another cronjob lifecycle operation is running.'
...
{
    for name in data_root state_root wrapper schedule cron_unit cron_line claude_path PATH HOME; do
        case "$name" in
            ...
            claude_path) value=$claude_path;; PATH) value=$PATH;; HOME) value=$HOME;;
        esac
        printf '%s=%s\n' "$name" "$(quote "$value")"
    done
} > "$transaction/config"
...
{
    printf '%s\n' '#!/bin/sh' '# my-toolbox claude-hi wrapper v1'
...
wrapper_stage=$(mktemp "$(dirname "$wrapper")/.claude-hi.XXXXXX")
...
say "Installed daily Claude Hi at $schedule. Run claude-hi to view status."
case ":$PATH:" in *":$HOME/.local/bin:"*) ;; *) say "Add $HOME/.local/bin to PATH to run claude-hi.";; esac
