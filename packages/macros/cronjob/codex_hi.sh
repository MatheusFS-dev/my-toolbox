...
state_root="${XDG_STATE_HOME:-$HOME/.local/state}/my-toolbox/codex-hi"
...
for path in "$data_root" "$state_root" "$wrapper" "$data_root/config" "$data_root/runtime.sh" "$state_root/events.log" "$state_root/latest-run.log" "$state_root/run.lock" "$state_root/install.lock"; do
