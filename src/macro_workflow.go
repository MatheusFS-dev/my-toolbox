...
func (workflow DefaultMacroWorkflow) run(macro Macro) error {
	...
	if goos == "linux" && goarch == "arm64" {
		return fmt.Errorf("Run is unavailable on Linux ARM64 because the AutoHotkey Linux port has no ARM64 runtime")
	}
	...
	for _, argument := range macro.Arguments {
		...
		if !ok {
			return fmt.Errorf("macro argument %q returned an invalid answer", argument.ID)
		}
		if strings.TrimSpace(value) == "" {
			value = argument.Default
		}
		if err := validateMacroArgument(argument, value); err != nil {
			return fmt.Errorf("invalid %s: %w", argument.Prompt, err)
		}
...
func autoHotkeyCandidates(goos, home string) []string {
	...
	if goos == "linux" {
		candidates = append(candidates, filepath.Join(home, ".local", "bin", "ahk"))
	} else {
...
func (workflow DefaultMacroWorkflow) install(goos, goarch, home string) error {
	...
	repository := "MonoEven/Autohotkey_Linux"
	if goos == "windows" {
	...
	if goos != "linux" || goarch != "amd64" {
		return fmt.Errorf("AutoHotkey installation is unsupported on %s/%s", goos, goarch)
	}
	archiveAsset, ok := findReleaseAsset(release, "-amd64.tar.gz")
	if !ok {
		return fmt.Errorf("latest Linux port release has no amd64 archive")
	}
	checksumAsset, ok := findReleaseAsset(release, "CKSUMS.txt")
	if !ok {
		return fmt.Errorf("latest Linux port release has no checksum file")
	}
	archive, archiveCleanup, err := downloadTemporary(client, archiveAsset.URL, ".tar.gz")
	if err != nil {
		return err
	}
	defer archiveCleanup()
	checksum, checksumCleanup, err := downloadTemporary(client, checksumAsset.URL, ".txt")
	if err != nil {
		return err
	}
	defer checksumCleanup()
	if err := verifyNamedChecksum(archive, checksum, archiveAsset.Name); err != nil {
		return err
	}
	directory, err := os.MkdirTemp("", "tb-ahk-install-")
	if err != nil {
		return err
	}
	defer os.RemoveAll(directory)
	...
	installer := filepath.Join(directory, "autohotkey-linux", "tools", "linux", "install.sh")
	command := exec.Command("sh", installer, "--prefix", filepath.Join(home, ".local"), "--yes")
	command.Stdout = workflow.Output
	command.Stderr = workflow.Output
	command.Stdin = os.Stdin
	if err := command.Run(); err != nil {
		return fmt.Errorf("install AutoHotkey Linux port: %w", err)
	}
	return nil
}
...
func verifyNamedChecksum(archive, checksum, name string) error {
	expected := ""
