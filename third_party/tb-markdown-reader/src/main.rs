...
struct Cli {
    ...
    /// the binary is piped to (e.g. `cat doc.md | tb-markdown-reader --section NAME`).
...
async fn main() -> Result<()> {
    ...
    if let Some(ref name) = cli.section {
        ...
        // inside a pipeline (e.g. `some-cmd | tb-markdown-reader --section NAME file.md`).
        //
    ...
    // ── stdin piping ─────────────────────────────────────────────────
    // When stdin is a pipe (`cat foo.md | tb-markdown-reader`), drain it to
